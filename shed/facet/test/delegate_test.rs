...
fn main() {
    ...
    let delegator = factory.build::<containers::Delegator>().unwrap();
    assert_eq!(delegator.one().get(), 1);

    let wrapped_delegator = factory.build::<containers::WrappedDelegator>().unwrap();
    assert_eq!(wrapped_delegator.one().get(), 1);
}
